...
def get_inputs_from_popup(hints: List[str], title: str = "请输入信息") -> Optional[List[str]]:
    ...
    tk.Button(btn_frame, text="取消", command=on_cancel, width=8).pack(side="left", padx=10)
...
def get_dynamic_inputs_from_popup(prompt: str, title: str = "请输入测量数据") -> Optional[List[float]]:
    """
    ...
    tk.Label(root, text=prompt, font=("Microsoft YaHei", 10), justify="left").pack(padx=10, pady=10)
    ...
    def on_ok(event=None):
        ...
        try:
            ...
            messagebox.showerror("错误", "存在无效数字，请确保仅输入数字并以空格或逗号分隔！")
...
def plot_comparison_spectrum(measured_wls: list, reference_wls: list, element_name: str, title: str="Spectrum Comparison"):
    ...
    def draw_lines(ax, wls, label_title):
        # 绘制背景
        ax.imshow(vis_rgb.reshape(1, -1, 3), aspect='auto', extent=[380, 780, 0, 0.04], alpha=0.3, zorder=1)
        
        for w in wls:
            rgb = wavelength_to_rgb(w)
            color = (rgb[0]/255, rgb[1]/255, rgb[2]/255) if rgb != (0,0,0) else (0.3, 0.3, 0.3)
            ax.vlines(w, 0, 1.0, colors=color, linewidth=2.5, zorder=3, capstyle='round')
            ax.annotate(f"{w:.1f}", xy=(w, 1.0), xytext=(0, 6), textcoords='offset points', 
